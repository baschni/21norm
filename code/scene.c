...
void	find_min_distance_for_point(double *d_min, t_vect *point, t_vect *center, t_scene *scene) //, double x, double y, double z, t_scene *cam)
{
	...
	temp = v_subst(point, center);
	p_proj = v_proj(temp, scene->dir, &d_point_center);
	// d_point_center = ft_abs(d_point_center);
	free(temp);
	x = fabs(v_mult(p_proj, scene->orient_x));
	y = fabs(v_mult(p_proj, scene->orient_y));
	printf("x: %f, y: %f\n", x, y);
	x = x / tan(scene->view_angle / 2) - d_point_center;
	y = y * scene->view_width / scene->view_height / tan(scene->view_angle / 2) - d_point_center;

	printf("distance from center: %f, d_x: %f, d_y: %f\n", d_point_center, x, y);
	// temp = v_proj(p_proj, scene->orient_y, &y);
	// y = scene->view_height / scene->view_width / tan(scene->view_angle / 2) * y - d_point_center;
	if (x > *d_min)
		*d_min = x;
	if (y > *d_min)
		*d_min = y;
...
t_scene	*find_cam_position(t_map *map, t_scene *scene)
{
	...
	while (row < map->height)
	{
		...
		find_min_distance_for_point(&d_min, temp, scene);//, row, col, *current, cam);
		free(temp);
		if ((++col) == map->width)
			row++;
		if (col == map->width)
...
void	s_free(t_scene *scene)
{
	...
	if (scene->center)
		v_free(scene->center);
	if (scene->edges3d)
		ft_lstclear(&scene->edges3d, v_free);
	free(scene);
}
